...
interface BenefitChecklistViewProps {
  benefit: BenefitRequest
  onBack: () => void
  onAskAboutBenefit: () => void
}
...
export function BenefitChecklistView({ benefit, onBack, onAskAboutBenefit }: BenefitChecklistViewProps) {
  const [items, setItems] = useState<ChecklistItem[]>(getChecklistForBenefit(benefit.type))
    ...
    <div className="flex h-full flex-col bg-gray-50">
      ...
      <div className="flex items-center justify-between border-b border-gray-200 bg-white px-6 py-4">
        <div className="flex items-center gap-3">
          <SidebarTrigger />
          <Button variant="ghost" size="sm" onClick={onBack} className="gap-2 hover:bg-gray-100">
            <ArrowLeft className="h-4 w-4" />
            Voltar
          </Button>
        </div>
        <Button onClick={onAskAboutBenefit} className="gap-2 bg-gray-900 hover:bg-gray-800 text-white" size="sm">
      ...
      <div className="flex-1 overflow-y-auto p-6">
        <div className="mx-auto max-w-4xl">
          {/* Title and Progress */}
          <div className="mb-8">
            <h1 className="mb-2 text-3xl font-bold text-gray-900">{benefit.name}</h1>
            <p className="text-gray-600 leading-relaxed">
              Siga cada etapa com calma. Clique em cada item para ver mais detalhes sobre o que fazer.
            </p>

            {/* Progress Bar */}
            <div className="mt-6 rounded-lg bg-white p-6 shadow-sm border border-gray-200">
              <div className="mb-3 flex items-center justify-between">
                <span className="text-sm font-semibold text-gray-700">Progresso da Solicitação</span>
          ...
          <Card className="mb-6 border-blue-200 bg-blue-50 p-4">
            <div className="flex gap-3">
              ...
              <div className="text-sm text-blue-900 leading-relaxed">
                ...
                <p>
                  Clique em cada etapa para ver instruções detalhadas. Marque como concluída conforme for avançando. Não
                  tenha pressa, faça uma etapa por vez.
                </p>
              </div>
            </div>
          </Card>
                    ...
                    <div className="flex items-center gap-3">
                      <span
                        className={`flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full text-sm font-bold ${
                          item.completed ? "bg-gray-300 text-gray-600" : "bg-gray-900 text-white"
                        }`}
                      >
                        {index + 1}
                      </span>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {expandedItem === item.id && (
                        <div className="mt-4 rounded-lg bg-gray-50 p-4 border border-gray-200">
                          <p className="text-sm font-semibold text-gray-900 mb-2">Como fazer:</p>
                          <p className="text-sm text-gray-700 leading-relaxed">{item.details}</p>
                        </div>
                      )}
                    </div>
                    {item.completed && <CheckCircle2 className="h-6 w-6 text-gray-400 flex-shrink-0" />}
                  </div>
                </div>
              </Card>
            ))}
          ...
          {/* Completion Message */}
          {progress === 100 && (
            <Card className="mt-8 border-0 bg-gradient-to-r from-gray-800 to-gray-900 p-6 text-white shadow-lg">
              <div className="flex items-start gap-4">
                <CheckCircle2 className="h-8 w-8 flex-shrink-0" />
                <div>
                  <h3 className="text-xl font-bold">Parabéns! Você completou todas as etapas</h3>
                  <p className="mt-2 text-gray-200 leading-relaxed">
                    Agora é aguardar o retorno do órgão responsável. Continue acompanhando o processo pelo número do
                    protocolo. Se tiver dúvidas, procure orientação no CRAS ou Defensoria Pública.
                  </p>
                </div>
              </div>
            </Card>
